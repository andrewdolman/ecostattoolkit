...
#Geoff Phillips
#Date:10 Aug 2016
#File name for script: TKit_fit_lin_mod2_DiffExample.R
...
FName<-"O:\\OneDrive\\Work\\Projects\\2016\\JRC_NutrientPhase2\\Data\\Analysis\\2016_08_10\\DataTemplate_L-CB2L_EC1.csv"
...
cc<-complete.cases(data$EQR,data$P,data$N)
data.cc<-data[cc,]
dim(data.cc)
...
x2<-10^x
y<-data.cc$EQR
z<-as.factor(data.cc$N_P_Cat)
#------------------------------------------------------------------------------------------
#             Assign data used for models to x.u and y.u
#------------------------------------------------------------------------------------------
nut.min<-20	#min nutrient value used for model
nut.max<-1000	#max nutrient value used for model 
...
z.u<-as.factor(data.cc$N_P_Cat[data.cc$P<=nut.max & 
	data.cc$P>=nut.min &
	data.cc$Exclude_P==FALSE])
...
print(Out5)
write.table(Out5,"clipboard",sep="\t",row.names=F,col.names=F)
#Now paste to Excel
