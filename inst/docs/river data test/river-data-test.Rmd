...
library(tidyverse)
river_data_veg_mean_station <- ndbrivers::river_data_veg_mean_station %>% 
  tbl_df()


bd <- ndbrivers::ecological_status_boundaries %>% 
  select(Boundary, Chla_DIN_seasonal_mean_µg_L, Region, River_type_WFD) %>% 
 # arrange(Chla_DIN_seasonal_mean_µg_L) %>%
  spread(Boundary, Chla_DIN_seasonal_mean_µg_L)

river_test_data <- river_data_veg_mean_station %>% 
  filter(is.na(TP_mg_L)==FALSE,
         is.na(Chla_µg_L)==FALSE,
         River_type_WFD %in% ndbrivers::phyto_relevant_types) %>% 
  left_join(., bd) %>% 
  mutate(BioClass_C = ifelse(Chla_µg_L <= `H/G`, "High", 
                           ifelse(Chla_µg_L <= `G/M`, "Good", 
                                  ifelse(Chla_µg_L <= `M/P`, "Moderate",
                                         ifelse(Chla_µg_L <= `P/B`, "Poor", "Bad")))),
         BioClass = as.numeric(factor(BioClass_C, 
                                      levels = rev(c("High", "Good", "Moderate", "Poor", "Bad")),
                                      ordered  =TRUE))) %>% 
   transmute(
    Record = 1:nrow(.),
    Unique_ID = Station,
    ...
    Exclude_N = 0,
    BioClass = BioClass,
    Group = River_type_WFD) 

data <- river_test_data

# create seperate .csv files for testing shiny app
for(i in 1:length(data$Group))
  
plyr::d_ply(data, "Group", function(x)
  write.csv(x, paste0("inst/extdata/types/River_type", unique(x$Group), ".csv"),
...
```{r, fig.width = 7, fig.cap="Figure A 8 Range of TP and TN concentrations for water bodies grouped by class"}
par(mfrow = c(1,2))
...
```{r, eval = FALSE}
plot(data$EQR ~ data$P, log="xy")#note the nutrient axis is set to a log scale
...
```{r, fig.width=6, fig.height = 8, fig.cap="Figure A 15 Output from TKit_fit_lm2 comparing models and showing ranges of (left) good/moderate and (right) high/good boundaries fitted to data grouped by lake type."}
